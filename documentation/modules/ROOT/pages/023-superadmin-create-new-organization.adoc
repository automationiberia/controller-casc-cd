...
* xref:#_automatically[Automatically]: A Job Template to create all the required components for a new regular organization is provided.
* xref:#_manually[Manually]: All the required components for a new regular organization are created step-by-step.
...
Automatically::
+
1. Locate the Job Template called `superadmin WF New Project` and Launch it.
+
...
3. Let's de execution to finish:
+

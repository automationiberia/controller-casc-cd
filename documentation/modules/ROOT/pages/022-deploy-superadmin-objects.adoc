...
That file may already be ignored in the contents of the `.gitignore`, so it can't be uploaded to the GitLab repository. See the xref:021-initial-dir-and-files.adoc#_recommended_basic_files[Recommended basic files]
====

[IMPORTANT]
====
Another important thing to remember is to add the public key part of the private key configured at `orgs_vars/$\{SUPERADMIN_ORG\}/env/dev/controller_credentials.d/controller_credentials_scm.yml` to the GitLab and GitHub repositories used by the configured projects. See the xref:021-initial-dir-and-files.adoc#_dev_source_control_manager_and_gitlab_api_token_credentials_will_let_the_job_templates_to_connect_to_the_gitlab_repositorygroup[(dev) Source Control Manager and GitLab API Token Credentials will let the Job Templates to connect to the GitLab repository/group]
====
...
[source,bash]
----
ansible-navigator run casc_ctrl_config.yml -i inventory -l dev -e "{orgs: ${SUPERADMIN_ORG}, dir_orgs_vars: orgs_vars, env: dev}" -m stdout --eei ${AUTOMATIONHUB_HOST}/ee-casc:latest --vault-password-file .vault_password --pull-arguments=--tls-verify=false
----

[IMPORTANT]
====
The execution above will fail because there are missing credentials to be assigned to the new organization. To workaround that, run the same command adding the flag `--tags credentials` and, when finished, run the original command again.
====

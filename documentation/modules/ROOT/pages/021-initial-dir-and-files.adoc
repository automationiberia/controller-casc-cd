...
* Go to the URL https://gitlab.com/-/profile/personal_access_tokens and create a new personal access token with *`api` scope*:
+
image::configure_gitlab_personal_access_token.png[]
+
NOTE: For more information related to the personal access token creation, take a look at https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html#create-a-personal-access-token[Create a personal access token on GitLab]
...
+
NOTE: For more information related to configure the SSH access to your GitLab account see: https://docs.gitlab.com/ee/user/ssh.html[Use SSH to communicate with GitLab]
...
# Controller hostname/IP (DEV):
export CONTROLLER_DEV_HOST=<CHANGE_ME>
# Controller host name/IP(PRO):
export CONTROLLER_PRO_HOST=<CHANGE_ME>
# Superadmin organization name:
export SUPERADMIN_ORG=<CHANGE_ME>
# Project URL for CasC (SSH format):

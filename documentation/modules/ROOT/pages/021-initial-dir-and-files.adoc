...
[%collapsible]
======
Alternatively to using your own existing SSH key...

[.console-input]
[source,bash]
----
ssh-key gen -f id_rsa_gitlab
----
======
...
Initial configuration files must be created. First of all, the connection credentials to the controllers are needed. They will be created under the `group_vars` directory as they might differ from one environment to another, and Ansible provides this method for the implementationfootnote:[https://docs.ansible.com/ansible/latest/inventory_guide/intro_inventory.html].
...
[source,bash]
----
export GITLAB_PRIV_KEY=$(cat id_rsa_gitlab) ## or the path to your own private key
export GITLAB_USER=<CHANGE_ME>

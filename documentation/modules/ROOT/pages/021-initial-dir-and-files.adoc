...
Additionally, a GitLab Runner instance must be created and configured to allow the GitLab Actions to be executed at `gitlab.com`. The previous step has already executed some actions to ease the configuration, so following are the remaining steps:

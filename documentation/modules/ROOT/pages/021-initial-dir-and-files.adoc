...
The organization directory should follow below structure:

* *`group_vars`*: Ansible directory that leverage to initially create and use the credentials to reach the Ansible Automation Controller.
* *`orgs_vars`*: Directory structure that contains YAML files created to have data models  all the organization object files.

[IMPORTANT]
======
Steps below are meant to be locally executed from the base directory of the repository.
======

== Pre-Requisites

An *empty GitLab repository* is required to follow this workshop, which is not provided. Additionally, set-up on your gitlab account a new token with `api` scope and configure a ssh key pair to access your account.

[NOTE]
=======

Take a look at Gitlab official documentation for repository access and configuration due are not under workshop scope. 

- https://docs.gitlab.com/ee/user/project/repository/[Create a repository on Gitlab.com]

- https://docs.gitlab.com/ee/user/ssh.html[Use SSH to communicate with GitLab]

- https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html#create-a-personal-access-token[Create a personal access token on GitLab]
=======

...
[IMPORTANT]
====
The default branch for that GitLab Repository MUST be the branch `pro`. This will be configured in the xref:021-initial-dir-and-files.adoc#_commit_and_push_the_initial_version_of_cac[next steps of the lab].
...
[IMPORTANT]
======
At this point, the GitLab repository can be configured to have the branch `pro` as the default one, as described <<gitlab-default-branch,here>>
======

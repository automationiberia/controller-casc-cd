...
[[gitops-branch]]
=== Create a new branch from `dev` to introduce the new items
...
[[mrdevtopro]]
=== Promote the `dev` branch to production (`pro` branch)

Similarly to the <<mrtodev,previous step>>, create a new Merge Request from the `dev` branch to the `pro` branch:
...
[[runjtpro]]
=== Run the new Job Template at PRO
...
Repeat the steps to create a new Merge Request from `dev` to `pro`, as described at <<mrdevtopro>>.

[[runjtpro-again]]
=== Run again the new Job Template at PRO

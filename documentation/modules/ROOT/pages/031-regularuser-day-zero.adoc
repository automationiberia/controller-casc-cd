...
5. [[mrtodev]]Create a Merge Request to `dev` branch
+
--
* Go to Merge Requests and create a new merge request
+
image::mrtodev.png[New merge request]

* Select the source branch `add_info_job_template` and `dev` as the destination one
+
image::newmrtodev.png[New merge request to dev]

* Fill in the merge request information
+
image::newmrtodevstep2.png[Fill in the merge request information]

* Merge the merge request
+
image::newmrtodevstep3.png[Merge the merge request]

'''

The following automated process has been executed at the Ansible Automation Controller:

image::devworkflowjobs.png[Look at the Ansible Automation Controller's jobs]

The following diagram shows the components of the workflow:

image::workflowdiagram.png[Workflow Diagram]

Of course, the new Job Template has been created:

image::newjtdev.png[New Job Template Dev]
--

6. [[mrdevtopro]]Promote the `dev` branch to production (`pro` branch)
+
--
Similarly to the <<mrtodev,step 5>>, create a new Merge Request from the `dev` branch to the `pro` branch:
...
[WARNING]
====
**Be sure to write a title that makes sense for the Merge Request**: The default value here is `dev`, that is not useful at all!
====
...
image::newjtpro.png[New Job Template Pro]
--

7. [[runjtpro]]Run the new Job Template at PRO
+
--
Run the Job Template:

image::launchjtpro.png[Run the JT]

and check that it is failing:

image::jtprofailed.png[Job Template PRO failed]
--
...
+
image::rollbackddv0.2.png[Run the JT casc-twitch-demo CasC_JobTemplates_AAP_Drop_Diff]

* Run the Job Template `casc-twitch-demo CasC_JobTemplates_AAP_CD_Config_Controller` with the previous working version:
+
image::rollbackccv0.2.png[Run the JT casc-twitch-demo CasC_JobTemplates_AAP_CD_Config_Controller]
--
...
Modify the Job Template to use the correct playbook: `playbook: "new_playbook2.yaml"`
...
image::newmrtodev-fix_playbook.png[Create New Merge Request from fix_playbook to Dev]
image::newmrtodev-fix_playbook-step2.png[New Merge Request from fix_playbook to Dev]
image::newmrtodev-fix_playbook-step3.png[Merge the Request]

Repeat the steps to create a new Merge Request from `dev` to `pro`, as described at <<mrdevtopro,step 6>>.
--

10. [[runjtpro-again]]Run again the new Job Template at PRO
+
--
...
image::jtprosuccess.png[Job Template PRO success]
--

...
+
Similarly to the xref:mrtodev[step 5], create a new Merge Request from the `dev` branch to the `pro` branch:
+
* Select the source branch as `dev` and `pro` as the destination one]
+
image:newmrtopro.png[New Merge Request Pro]
...
9. Fix your playbook
   ...
   * image:newmrtodev-fix_playbook-step3.png[Merge the Request]
   ...
   Repeat the steps to create a new Merge Request from `dev` to `pro`, as described at xref:mrdevtopro[step 6]
...
10. Run again the new Job Template at PRO <a name="runjtpro"></a>
    ...
    image:jtprosuccess.png[Job Template PRO failed]
